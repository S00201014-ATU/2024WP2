...
export class ProductService {
  private apiUrl = 'https://two024wp2-backend.onrender.com/products';
  ...
  getProducts(): Observable<Product[]> {
    return this.http.get<Product[]>(this.apiUrl); // No need for auth headers for fetching products
  }

  // Fetch a product by its ID
  getProductById(id: string): Observable<Product> {
    return this.http.get<Product>(`${this.apiUrl}/${id}`); // No need for auth headers for fetching a product by ID
  }
